...
public:
    ...
    float currentY() {
        return ((max.y - min.y) / 2); // size div 2 -- the middle of the object
    }
    // convert data to degrees
    void toDegrees() {
        if (vecX.x) {
            float loc = vecX.degrees;
            vecX.degrees = ofMap(abs(loc), min.x, max.x, range.x, range.y); // center is 0, looking at the bottom is 90
            if (loc > 0) {
